...
void dogm_printLine( uint32_t line_number, char *str ){
    
    dogm128_set_cursor(0, 56-(line_number*8));
    dogm128_print_string(str);
    dogm128_update_display();
}

void _delay_ms( uint32_t ile );
void _delay_ms( uint32_t ile )
{
	uint32_t k,i;
	for ( k=0;k<ile;k++) {
...
void dogm128_init(void)
{
	...
	//dogm128_send_command(DOGM128_ADC_NORMAL);
	...
	//dogm128_send_command(DOGM128_BIAS_17);
	...
	dogm128_send_command(0x10); /* Contrast */
